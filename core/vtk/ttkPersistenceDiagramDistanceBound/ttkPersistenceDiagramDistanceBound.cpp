...
#include "ttkPersistenceDiagramDistanceBound.h"
#include <ttkUtils.h>

vtkStandardNewMacro(ttkPersistenceDiagramDistanceBound);

ttkPersistenceDiagramDistanceBound::ttkPersistenceDiagramDistanceBound() {
  this->SetNumberOfInputPorts(1);
  this->SetNumberOfOutputPorts(1);
}

int ttkPersistenceDiagramDistanceBound::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkPersistenceDiagramDistanceBound::FillOutputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkPersistenceDiagramDistanceBound::RequestData(vtkInformation *ttkNotUsed(request),
  ...
  const std::string arrayName = "L-distance";
