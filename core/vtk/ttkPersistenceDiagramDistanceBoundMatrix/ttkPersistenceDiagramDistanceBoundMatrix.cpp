#include <ttkPersistenceDiagramDistanceBoundMatrix.h>
#include <ttkUtils.h>
...
vtkStandardNewMacro(ttkPersistenceDiagramDistanceBoundMatrix);

ttkPersistenceDiagramDistanceBoundMatrix::ttkPersistenceDiagramDistanceBoundMatrix() {
  SetNumberOfInputPorts(1);
  SetNumberOfOutputPorts(1);
}

int ttkPersistenceDiagramDistanceBoundMatrix::FillInputPortInformation(int port,
                                                 vtkInformation *info) {
  if(port == 0) {
...
int ttkPersistenceDiagramDistanceBoundMatrix::FillOutputPortInformation(int port,
                                                  vtkInformation *info) {
  if(port == 0) {
...
template <typename T>
int ttkPersistenceDiagramDistanceBoundMatrix::dispatch(
  ...
  return this->execute(distanceMatrix, triangulation, inputPtrs, nPoints);
}

int ttkPersistenceDiagramDistanceBoundMatrix::RequestData(vtkInformation * /*request*/,
                                    vtkInformationVector **inputVector,
                                    vtkInformationVector *outputVector) {
  ttk::Timer tm{};

  const auto blocks = vtkMultiBlockDataSet::GetData(inputVector[0], 0);
  // vtkUnstructuredGrid *temp = vtkUnstructuredGrid::SafeDownCast(blocks->GetBlock(0));

  ttk::Triangulation *triangulation = ttkAlgorithm::GetTriangulation(vtkDataSet::SafeDownCast(blocks->GetBlock(0)));
  ...
  switch(dataType) {
    vtkTemplateMacro(this->dispatch<VTK_TT>(distMatrix, triangulation,inputData, nPoints));
  }
