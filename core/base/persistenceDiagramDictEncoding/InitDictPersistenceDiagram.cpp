...
void InitFarBorderDict::execute(std::vector<Diagram> &DictDiagrams,
  ...
  if(nbAtoms == 2) {
    ...
    int Id2 = std::max_element(allDists[Id1].begin(), allDists[Id1].end())
              - allDists[Id1].begin();
  ...
  if(nbAtoms == 3) {
    ...
    int Id3 = std::max_element(distsTo2Pts.begin(), distsTo2Pts.end())
              - distsTo2Pts.begin();
  ...
  if(nbAtoms == 4) {
    ...
    Diagram atom3 = datas[Id3];
    DictDiagrams.push_back(atom3);
    int Id4 = std::max_element(allDists[Id3].begin(), allDists[Id3].end())
              - allDists[Id3].begin();
  ...
  if(nbAtoms == 5){
    ...
    for(int i = 0 ; i < nDiags ; ++i){
      if(i == Id1 || i == Id2 || i == Id3 || i == Id4){
        ...
        auto &dataMaxAtom2 = bidder_diagrams_max[Id2];
        auto &dataSadAtom2 = bidder_diagrams_sad[Id2];
        auto &dataMinAtom3 = bidder_diagrams_min[Id3];
        auto &dataMaxAtom3 = bidder_diagrams_max[Id3];
        auto &dataSadAtom3 = bidder_diagrams_sad[Id3];
        auto &dataMinAtom4 = bidder_diagrams_min[Id4];
        auto &dataMaxAtom4 = bidder_diagrams_max[Id4];
        auto &dataSadAtom4 = bidder_diagrams_sad[Id4];
        auto &dataMin = bidder_diagrams_min[i];
        auto &dataMax = bidder_diagrams_max[i];
        auto &dataSad = bidder_diagrams_sad[i];
        distsTo4Pts[i] = computeDistance(dataMinAtom1, dataMin)
                         + computeDistance(dataMaxAtom1, dataMax)
                         + computeDistance(dataSadAtom1, dataSad)
                         + computeDistance(dataMinAtom2, dataMin)
                         + computeDistance(dataMaxAtom2, dataMax)
                         + computeDistance(dataSadAtom2, dataSad)
                         + computeDistance(dataMinAtom3, dataMin)
                         + computeDistance(dataMaxAtom3, dataMax)
                         + computeDistance(dataSadAtom3, dataSad)
                         + computeDistance(dataMinAtom4, dataMin)
                         + computeDistance(dataMaxAtom4, dataMax)
                         + computeDistance(dataSadAtom4, dataSad);
      }
    }
    int Id5 = std::max_element(distsTo4Pts.begin() , distsTo4Pts.end()) -
    distsTo4Pts.begin();
