...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  switch(this->Distance){
    case DistanceType::SLICEDWASSERSTEIN: {
      ...
      if(this->do_min_) {
        setCurrentDiagrams(
          inputDiagramsMin, currentInputDiagramsMin, maxDiagPersistence);
      }
      if(this->do_sad_) {
        setCurrentDiagrams(
          inputDiagramsSad, currentInputDiagramsSad, maxDiagPersistence);
      }
      if(this->do_max_) {
        setCurrentDiagrams(
      ...
      for(size_t i = 0; i < nInputs[0]; ++i) {
        ...
        const auto getDist = [&](const size_t a, const size_t b, int &projNumber) -> double {
          ...
          if(this->do_sad_) {
            ...
            distance += sliceComputer.execute(disad, djsad, projNumber,SampleNumber);
          }
          if(this->do_max_) {
            auto &dimax = currentInputDiagramsMax[a];
            auto &djmax = currentInputDiagramsMax[b];
            distance += sliceComputer.execute(dimax, djmax, projNumber,SampleNumber);
          }
...
void PersistenceDiagramDistanceMatrix::setCurrentDiagrams(
